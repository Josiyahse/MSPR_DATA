...
def clean_and_transform_data():
  df_final_combined = pd.DataFrame()  # DataFrame vide pour combiner les données de toutes les années
  years = ['22', '17']  # Liste des années à traiter
  ...
  for year_suffix in years:
    ...
    # Set the year value to the parameter passed (either '22' or '17')
    df_merged['annee'] = year_suffix

    # Convert the year to a complete string format
    df_merged['annee'] = '20' + year_suffix
    ...
    # Ajouter les données de l'année courante au DataFrame combiné
    df_final_combined = pd.concat([df_final_combined, df_merged])



  return df_final_combined.drop_duplicates(subset=['department_id', 'year'])
