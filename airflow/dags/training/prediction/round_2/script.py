...
dag = DAG(
  'decision_tree_prediction_pipeline_round_2',
...
def predict_winners(**kwargs):
  ...
  for year in range(2027, 2038, 5):  # 2027, 2032, 2037
    ...
    future_data.to_sql(f'predicted_votes_{year}_round_2', con=engine, if_exists='replace', index=False)
    ...
    total_predicted_votes_by_candidate.to_sql('predicted_votes_by_candidate_and_year_round_2', con=engine,
                                              if_exists='append',
                                              index=False)
...
predict_winners_task = PythonOperator(
  task_id='predict_winners_round_2',
